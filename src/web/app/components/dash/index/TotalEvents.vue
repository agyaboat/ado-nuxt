  ...
  <CVCard class="flex items-center rounded-xl">
    <div class="flex gap-5">
      <div class="flex items-center">
        <div class="flex items-center p-3 rounded-lg border border-cv-sky bg-cv-sky">
          <span class="material-symbols-outlined text-cv-sky">calendar_month</span>
        </div>
      </div>
      <div class="grow">
        <div class="text-muted-color mb-1">TOTAL EVENTS CREATED</div>
        ...
        <div class="font-bold text-2xl flex justify-between items-center" v-else>
          <div>{{ summary.data.events }}</div>
          <div>
            <Button label="view" variant="link" @click="navigateTo({name: 'dash-events'})" />
          </div>
        </div>
      </div>
    </div>
